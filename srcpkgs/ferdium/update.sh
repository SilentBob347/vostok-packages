...
echo "Fetching latest Ferdium version..."

CURL_ARGS=(-fsSL -H "Accept: application/vnd.github+json")
[ -n "${GITHUB_TOKEN:-}" ] && CURL_ARGS+=(-H "Authorization: Bearer ${GITHUB_TOKEN}")

INFO=$(curl "${CURL_ARGS[@]}" \
    "https://api.github.com/repos/ferdium/ferdium-app/releases/latest") || {
...
if [ "$CURRENT" = "$LATEST" ]; then
    echo "ferdium: $CURRENT — already up to date"
    exit 0
fi

echo "ferdium: $CURRENT → $LATEST"

DEB_URL="https://github.com/ferdium/ferdium-app/releases/download/v${LATEST}/Ferdium-linux-${LATEST}-amd64.deb"
echo "URL: $DEB_URL"
