#!/bin/bash
# Auto-updater for protonmail-bridge
set -euo pipefail
...
echo "Fetching latest Proton Mail Bridge version..."
...
if [ "$CURRENT" = "$LATEST" ]; then
    echo "protonmail-bridge: $CURRENT — already up to date"
    exit 0
fi

echo "protonmail-bridge: $CURRENT → $LATEST"

DEB_URL="https://github.com/ProtonMail/proton-bridge/releases/download/v${LATEST}/protonmail-bridge_${LATEST}-1_amd64.deb"
echo "URL: $DEB_URL"
echo "Computing checksum..."
CHECKSUM=$(curl -L -# "$DEB_URL" | sha256sum | cut -d' ' -f1)

if [[ ! "$CHECKSUM" =~ ^[0-9a-f]{64}$ ]]; then
    echo "ERROR: Downloaded file is invalid (checksum not obtained)" >&2
    exit 1
fi
