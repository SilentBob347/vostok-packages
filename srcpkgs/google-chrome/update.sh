#!/bin/bash
# Auto-updater for google-chrome (RPM metadata, gzip handling)
set -euo pipefail

TEMPLATE="$(dirname "$0")/template"
CURRENT=$(grep '^version=' "${TEMPLATE}" | cut -d= -f2)

echo "Fetching latest Google Chrome version..."
...
if [ -z "$LATEST_VER" ]; then
    echo "ERROR: Could not find google-chrome-stable" >&2
    exit 1
fi
...
echo "google-chrome: ${CURRENT} → ${VERSION} (release ${RELEASE})"

DEB_URL="https://dl.google.com/linux/chrome/deb/pool/main/g/google-chrome-stable/google-chrome-stable_${VERSION}-${RELEASE}_amd64.deb"
echo "URL: ${DEB_URL}"
echo "Computing checksum..."
CHECKSUM=$(curl -L -# "${DEB_URL}" | sha256sum | cut -d' ' -f1)

sed -i "s/^version=.*/version=${VERSION}/" "${TEMPLATE}"
sed -i "s|^distfiles=.*|distfiles=\"https://dl.google.com/linux/chrome/deb/pool/main/g/google-chrome-stable/google-chrome-stable_\${version}-${RELEASE}_amd64.deb\"|" "${TEMPLATE}"
